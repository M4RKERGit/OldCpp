...
void main()
{
	vector<Perem> avto(10, { 0, "", -1, "OFF", 0 });
	vector<Perem> dynamic(10, { 0, "", -1, "OFF", 0 });
	...
	if ((fp = fopen(path, "r")) != NULL)
	{
		...
		fscanf_s(fp, "%d", &count_p);

		for (i = 0; i < count_p; i++)
		{
			fscanf_s(fp, "%d %s %d %s %s \n", &avto[i].Value, avto[i].Name, &avto[i].Pointer,
				avto[i].PSwitch, avto[i].Cell);
		}
	}
